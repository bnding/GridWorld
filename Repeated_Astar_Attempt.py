...
def a_star():
    ...
    global g
    global n
    while (list(open_list)!=[] and g[goal_state[0]][goal_state[1]]>list(open_list)[0][0]):
        ...
        for a in actions(state):
            ...
            if (g[next_state[0]][next_state[1]]>g[state[0]][state[1]]+1):
                ...
                #if next_state is not in obstacle_states, insert next_state into open with new f value
...
def main():
    ...
    while (start_state!=goal_state):
        ...
        heapq.heappush(open_list,(g[start_state[0]][start_state[0]]+h((start_state[0],start_state[1])),(start_state[0],start_state[1])))
        a_star()
        if (list(open_list)==[]):

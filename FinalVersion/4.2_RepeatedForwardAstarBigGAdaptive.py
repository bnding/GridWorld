...
from BinaryHeap_bigG import MinBinaryHeap_bigG
...
# get the current node's children list  
...
def astar(maze, start_node, end_node, block_tag, counter):
    ...
    global minbinaryheap_open_list
    global closed_list

    # Loop until you find the end
    while len(minbinaryheap_open_list) > 0:

        # Get the current node
        current_node = minbinaryheap_open_list.get_min_node()
        ...
        # expand current node: 
        # pop current off the open list, add to the closed list
        current_node = minbinaryheap_open_list.pop()
        ...
        for child in children:
            ...
            if is_first_encounter == 1:
                minbinaryheap_open_list.push(child)
                continue

            # check if the "child" state is in the open_list
            # "replace" the element in open_list with the "child" if 
            #   1) the "child" state is in open_list and 
            #   2) child.g < element.g
            #   >> keep the one with the low g(s) value in open_list
            matched_node_index, matched_node = minbinaryheap_open_list.find_node(child)
            if matched_node and (child.g < matched_node.g):
                minbinaryheap_open_list.remove_element(matched_node_index)
                minbinaryheap_open_list.push(child)
...
#       ** only use heuristic override from prior search  
#       (1) clean up heuritics h(s) override from prior search
#       (2) update heuritics h(s) for all nodes in closed_list (current search) 
#           >> with a more accurate heuristics
#           >> for next search to use 
# -----------------------------------------------------------------------------------------
...
minbinaryheap_open_list = MinBinaryHeap_bigG()
closed_list = []

def main():

    # file output
    outfile = open('RepeatedForwardAstarBigGAdaptive_50GridRuns.log','w+')
    print('-----------------------------------------------------------------------------------------', file=outfile)
    print('** Algorithm: Adaptive A* (use Big-G to break ties)', file=outfile)
    print('**            >> Adaptive A* is an Improved Repeated Forward A* (use Big-G to break ties)', file=outfile)
    ...
    total_agent_path_len_all_cases = 0          # for calculating the average agent path length per test case
    total_search_count_all_cases = 0            # for calculating the average search count per test case

    random_test_case = TestCaseRandom()

    # for test_case_index in range(len(random_test_case)):      # run 50 random cases for the algorithm comparison (case index: 0 to 49)
    for test_case_index in range(0,1):                          # run 1 case 
        ...
        minbinaryheap_open_list.cleanup_heap()
        closed_list = []
        ...
        while start != end:
            ...
            # switch the start and end states (Repeated Backward A* only)
            # tmp_position = start
            # start = end
            # end = tmp_position
            ...
            # Add the start node
            minbinaryheap_open_list.push(start_node)
            ...
            if (len(minbinaryheap_open_list) == 0) or (not path):
                ...
                if (len(minbinaryheap_open_list) == 0):
            ...
            # switch back between the "start" and "end" states (Repeated Backward A* only)
            # tmp_position = start
            # start = end
            # end = tmp_position

            # print the path >> after astar()
            print()
            print('>>> invoke astar()...')
           ...
           # function: move_agent_from_start_to_goal()
            ...
            # 2) if a blocked cell is encountered, return agent_position right before the blocked cell
            ...
            if agent_position == end:
                ...
                # append the path to the grand_path
                for i in path: 
                    if i != agent_position:
                        grand_path.append(i) 
                    else:
                        break
                # collect the expanded_cell_count
                grand_expanded_cell_count = grand_expanded_cell_count + len(closed_list)

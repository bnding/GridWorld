...
def a_star():
    ...
    while (g[start_state[0]][start_state[1]]>min()):
        state=open_list[min()]
...
def main():
    ...
    while (start_state!=start_state):
        counter=counter+1
        g[goal_state[0]][goal_state[1]]=0
        search[goal_state[0]][goal_state[1]]=counter
        g[start_state[0]][start_state[1]]=math.inf
        search[start_state[0]][start_state[1]]=counter
        open_list=[]
        closed_list=[]
        start_node = Node(None, goal_state)
        end_node = Node(None, start_state)
        open_list.append((g[goal_state[0]][goal_state[0]]+h((goal_state[0], goal_state[1])),(goal_state[0], goal_state[1])))
        a_star()
        if (open_list==[]):
